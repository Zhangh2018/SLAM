...
void Optimizer::BundleAdjustment(Map& m, int iter, int slice) {
    ...
    std::cout << "Points: " << points.size() << " Frames: " << frames.size()
              << std::endl;
    ...
    for (size_t i = 0; i < ptsIdx.size(); i++) {
        int idx = ptsIdx[i];
        Point* pt = points[idx];
        g2o::VertexSBAPointXYZ* Point = static_cast<g2o::VertexSBAPointXYZ*>(
            optimizer.vertex(pt->id + maxId + 1));
