...
void Init::triangulate(const cv::KeyPoint& keypoints1,
                       const cv::KeyPoint& keypoints2, cv::Mat& P1, cv::Mat& P2,
                       cv::Mat& points3d) {
    cv::Mat A(4, 4, CV_32F);
...
void Init::triangulate(const cv::Point2f& point1, const cv::Point2f& point2,
                       cv::Mat& P1, cv::Mat& P2, cv::Mat& point3d) {
    cv::Mat A(4, 4, CV_32F);
...
void Init::decomposeE(const cv::Mat& E, cv::Mat& R, cv::Mat& t) {
    ...
    R1 = U * W * Vt;
    R2 = U * W.t() * Vt;
    if (R1.at<float>(0, 0) + R1.at<float>(1, 1) + R1.at<float>(2, 2) >
        R2.at<float>(0, 0) + R2.at<float>(1, 1) + R2.at<float>(2, 2))
        R = R1;
    else
        R = R2;
...
Init::Init(cv::VideoCapture& _cap, cv::VideoCapture& _cap1,
           const int _nfeatures, const int _thold, float _H, float _W) {
    cap = _cap;
    cap1 = _cap1;
    nfeatures = _nfeatures;
    thold = _thold;
    detector = cv::FastFeatureDetector::create(thold);
    desc = cv::ORB::create(nfeatures, 1.2f, 8, 31, 0, 2, cv::ORB::FAST_SCORE,
                           31, thold);
...
void Init::extractKeyPoints(cv::Mat frame, std::vector<cv::KeyPoint>& keypoints,
                            cv::Mat& descriptors) {
    ...
    cv::Mat mask;
    // desc->detectAndCompute(frame, mask, keypoints, descriptors);
}
...
Matches* filterMatches(std::vector<cv::KeyPoint>& keypoints1,
    ...
    F = cv::findFundamentalMat(pMatches1, pMatches2, mask, cv::FM_RANSAC, 3.0f,
                               0.99f);
...
void Init::processFrames(Map& m) {
    ...
    while (1) {
        ...
        cv::Mat F;
        goodMatchesCurrent =
            filterMatches(keypoints1, keypoints2, descriptors1, matches, F);
        ...
        matches.clear();
        matcher->knnMatch(goodMatchesPrev->desc, goodMatchesCurrent->desc,
                          matches, 2);
        delete (goodMatches);
        ...
        // std::cout << cv::format(poseTest.back(), cv::Formatter::FMT_PYTHON)
        // << std::endl;
        P2 = K * P2;
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            if (xyz[2] < th || cv::norm(xyz[2] - th) > 20)
                continue;
        ...
        std::cout << "Points size: " << points.size() << std::endl
                  << "rPoints size: " << rPoints.size() << std::endl;
        ...
        for (size_t i = 0; i < goodMatches->mask.size(); ++i) {
            ...
            if (!rPoints.empty()) {
                ...
                for (size_t j = 0; j < rPointsIdx.size(); j++) {
                    int k = rPointsIdx[j];
                    double distance =
                        cv::norm(points[k]->getDesc(), goodMatches->desc.row(i),
                                 cv::NORM_HAMMING);
                    float eDistance =
                        euclideanDistance(rPoints[j], goodMatches->kp[i]);
                ...
                if (flag) {
                    points[mPoint]->addObservation(keyframe,
                                                   keyframe->getKpSize());
                    keyframe->addKeypoint(goodMatches->kp[i],
                                          points[mPoint]->id);
                    countMappedPoints++;
            ...
            triangulate(goodMatchesPrev->kp[i], goodMatchesCurrent->kp[i], P1,
                        P2, s3DPoint);
            // checking if coordinates goes to infinity
            if (!std::isfinite(s3DPoint.at<float>(0, 0)) ||
                !std::isfinite(s3DPoint.at<float>(1, 0)) ||
                !std::isfinite(s3DPoint.at<float>(2, 0)))
                continue;
            ...
            if (s3DPoint.at<float>(2, 0) <= 0 && cosParallax < 0.99998)
                continue;

            cv::Mat s3DPoint2 = R * s3DPoint + t;
            // if (s3DPoint2.at<float>(2) <= 0) continue;
            if (s3DPoint2.at<float>(2, 0) <= 0 && cosParallax < 0.99998)
                continue;
            ...
            if (reprojErr(kp1x, kp1y, cx, cy, f, s3DPoint))
                continue;

            // Check reprojection error in second image
            if (reprojErr(kp2x, kp2y, cx, cy, f, s3DPoint2))
                continue;

            if (s3DPoint.at<float>(2) > th &&
                cv::norm(s3DPoint.at<float>(2) - th) < 20) {
                cv::Vec3b intensity = frame.at<cv::Vec3b>(
                    cv::Point(goodMatchesCurrent->kp[i].pt.x,
                              goodMatchesCurrent->kp[i].pt.y));
                std::vector<float> color = {
                    static_cast<float>(intensity.val[2]) / 255,
                    static_cast<float>(intensity.val[1]) / 255,
                    static_cast<float>(intensity.val[0]) / 255};
                Point* pt = new Point(
                    m.getPointsSize(), s3DPoint.at<float>(0, 0),
                    s3DPoint.at<float>(1, 0), s3DPoint.at<float>(2, 0),
                    goodMatches->desc.row(i), color);
        ...
        std::cout << "Distance: " << aD / distanceCnt
                  << " Euclidian Distance: " << aED / distanceCnt << std::endl;
        std::cout << "Number of mapped points:    " << countMappedPoints
                  << std::endl;
        std::cout << "Number of new map points:   " << countNewPoints
                  << std::endl;
        std::cout << "Number of map points:       " << m.getPointsSize()
                  << std::endl;

        // if (keyframe->getKpSize() < 100) keyframe->bad = true;
        m.addFrame(keyframe);

        std::cout << "good matches after tests:   " << goodMatches->mt.size()
                  << std::endl;
        ...
        if (frames.size() % iterations == 0 && frames.size() > 0) {
            ...
            // t1.join();
            optimizer.BundleAdjustment(m, 10, 50);
        }

        drawMatches(frame, goodMatchesPrev->kp, goodMatchesCurrent->kp,
                    goodMatches->mt);
...
void Init::drawMatches(cv::Mat& frame, std::vector<cv::KeyPoint>& keypoints1,
                       std::vector<cv::KeyPoint>& keypoints2,
...
void Init::normalize(std::vector<cv::KeyPoint>& points,
                     std::vector<cv::KeyPoint>& normalizedPoints, cv::Mat& T) {
    float mX = 0, mY = 0, d = 0;
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        // d += std::sqrt(pow(normalizedPoints[i].pt.x, 2) +
        // pow(normalizedPoints[i].pt.y, 2));
    }
...
bool Init::reprojErr(float kpx, float kpy, float cx, float cy, float f,
                     cv::Mat point) {
    float imx, imy;
